...
1. 建立起处理patent data的一套统一方案: 
	a. collapse之前，需要将几个关键词处理一下： 
		...
		- “波浪能”，“海洋能”与“潮汐”
		- “风能”与“风电”
	b. 通过collapse: 对于每个关键词{i}，每个地区{p}，每个申请时间{t}（三个维度）加总： ni_{pt}
	c. 跨类别不能比较的问题： 
...
/*
foreach v of varlist p_* {
by carbon_market atime, sort : egen float carbon_market_`v'_c = total(`v'_c)
twoway (lpoly carbon_market_`v'_c t if carbon_market==1 & t>=10) (lpoly carbon_market_`v'_c t if carbon_market==0 & t>=10), legend(order(1 "Carbon Market == 1" 2 "No" )) title("`v'")
graph export "$root\figures\carbon_market_`v'_trend.png", as(png) replace
}
*/
